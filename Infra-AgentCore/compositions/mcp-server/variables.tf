...
variable "memory" {
  ...
  description = "Memory config. mcp-server actualmente no usa memory; se mantiene para alineación con compositions de agent-*."
}
...
variable "gateway_policies" {
  type = list(object({
    gateway        = string
    attach_mode    = optional(string, "LOGONLY")
    cedar_policies = list(string)
  }))
  description = "Cedar policies opcionales por gateway. Solo se aplican si features.enable_policies=true."
  default     = []
}
